...
SUPPORTED_24x80 = ['IBM-3179-2', 'IBM-3196-A1', 'IBM-5292-2', 'IBM-5291-1', 'IBM-5251-11']
...
class RobotEngine:
    ...
    def run_tmux(self, args: List[str]) -> str:
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"Tmux command failed: [{' '.join(cmd)}]. Error: {error_output.strip()}")
    ...
    def find_text_in_buffer(self, pane_content: str, text: str, row=None, col=None, end_row=None, end_col=None, is_message_line=None) -> bool:
        lines = pane_content.splitlines()
    ...
    def wait_for_text_internal(self, text: str, timeout: int, row=None, col=None, end_row=None, end_col=None, is_message_line=None) -> Tuple[bool, str]:
        ...
        while time.time() < expiry:
            ...
            if self.find_text_in_buffer(last_content, text, row, col, end_row, end_col, is_message_line):
                return True, last_content
            time.sleep(0.5)

        return False, last_content

    def wait_for_text(self, text: str, timeout: int, row=None, col=None, end_row=None, end_col=None, is_message_line=None):
        found, last_content = self.wait_for_text_internal(text, timeout, row, col, end_row, end_col, is_message_line)
        if found:
            return
    ...
    def capture_debug_screen(self, action_name: str):
        ...
        try:
            ...
            logger.debug(f"[Debug Capture] Screen saved to {os.path.relpath(save_path)}")
