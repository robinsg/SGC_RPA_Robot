...
def test_cli_missing_args():
    with patch("sys.argv", ["cli.py"]), patch("sys.exit") as mock_exit:
...
def test_cli_success(tmp_path, monkeypatch):
    ...
    with patch("sys.argv", ["cli.py", "--yaml-file", str(yaml_file)]), patch(
        "robot_py.engine.RobotEngine.run"
    ) as mock_run, patch(
        "robot_py.engine.RobotEngine.check_session_exists", return_value=True
    ):
        main()
...
def test_cli_error_handling(tmp_path, monkeypatch):
    ...
    with patch("sys.argv", ["cli.py", "--yaml-file", "non_existent.yaml"]), patch(
        "sys.exit"
    ) as mock_exit:
...
def test_cli_load_env(tmp_path, monkeypatch):
    ...
    with patch(
        "sys.argv", ["cli.py", "--yaml-file", str(yaml_file), "--env", str(env_file)]
    ), patch("robot_py.engine.RobotEngine.run"), patch(
        "robot_py.engine.RobotEngine.check_session_exists", return_value=True
    ):
        main()
...
def test_cli_env_not_found(tmp_path):
    ...
    with patch(
        "sys.argv",
        ["cli.py", "--yaml-file", str(yaml_file), "--env", "non_existent.env"],
