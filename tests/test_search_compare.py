...
def test_search_and_compare_failure_line(engine, capsys):
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "wait_for_text_internal", return_value=(False, pane_content)
    ), patch.object(engine, "execute_steps") as mock_execute_steps, patch(
        "robot_py.engine.logger.info"
    ) as mock_logger:
        ...
        mock_logger.assert_any_call(
            f'Search for text failed to find "{action.text}". Actual value found: "{actual_content}".'
        )
...
def test_search_and_compare_failure_positional(engine, capsys):
    ...
    action = SearchAndCompareAction(
        type="search_and_compare", text="Dummy Text", row=9, col=10
    )
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "wait_for_text_internal", return_value=(False, pane_content)
    ), patch("robot_py.engine.logger.info") as mock_logger:

        engine.execute_step(action)

        # It should extract the length of the search string (Dummy Text = 10 chars)
        mock_logger.assert_any_call(
            f'Search for text failed to find "{action.text}". Actual value found: "{actual_at_pos}".'
        )
...
def test_engine_run_termination(engine):
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "capture_pane", return_value=""
