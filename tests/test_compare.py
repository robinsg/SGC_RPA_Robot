...
def test_compare_numeric_conversion_error_actual(engine):
    action = CompareAction(
        type="compare", expected="40", operator="EQ", row=1, col=1, length=5
    )
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        ...
        with pytest.raises(
            ...
            match="Compare data is incompatible: Extracted value 'ABCDE' is not numeric",
        ):
            engine.execute_step(action)
...
def test_compare_numeric_conversion_error_expected(engine):
    action = CompareAction(
        type="compare", expected="FORTY", operator="EQ", row=1, col=1, length=2
    )
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        ...
        with pytest.raises(
            TerminationException,
            match="Compare data is incompatible: The expected value 'FORTY' resolved to 'FORTY', which is not numeric",
        ):
            engine.execute_step(action)
...
def test_compare_numeric_conversion_error_variable(engine):
    ...
    action = CompareAction(
        type="compare", expected="{{SEC_LEVEL}}", operator="EQ", row=1, col=1, length=2
    )
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        ...
        with pytest.raises(
            TerminationException,
            match="Compare data is incompatible: The run time variable '{{SEC_LEVEL}}' resolved to 'XX', which is not numeric",
        ):
            engine.execute_step(action)
...
def test_engine_run_termination_logging(engine):
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "capture_pane", return_value="40" + "\n" * 23
