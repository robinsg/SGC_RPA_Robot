...
def test_find_text_in_buffer_block(mock_engine):
    ...
    assert (
        mock_engine.find_text_in_buffer(
            pane_content, "TARGET", row=2, col=5, end_row=2, end_col=10
        )
        is True
    )
    assert (
        mock_engine.find_text_in_buffer(
            pane_content, "TARGET", row=2, col=1, end_row=2, end_col=4
        )
        is False
    )
...
def test_find_text_in_buffer_message_line(mock_engine):
    ...
    pane_content = "\n".join(lines)
    assert (
        mock_engine.find_text_in_buffer(pane_content, "Error", is_message_line=True)
        is True
    )
    assert (
        mock_engine.find_text_in_buffer(pane_content, "line", is_message_line=True)
        is False
    )
...
def test_find_text_in_buffer_message_line_132(tmp_path, monkeypatch):
    ...
    # To be detected as 27x132, line 28 must have '/' at col 76
    lines.append(" " * 75 + "/")
    pane_content = "\n".join(lines)
    assert (
        engine.find_text_in_buffer(pane_content, "Error", is_message_line=True) is True
    )
...
def test_find_text_in_block(mock_engine):
    ...
    match_row = mock_engine.find_text_in_block(
        pane_content, "Target", row=1, col=1, end_row=3, end_col=80
    )
    assert match_row == 2

    # Search for "Target" starting from row 3
    match_row = mock_engine.find_text_in_block(
        pane_content, "Target", row=3, col=1, end_row=4, end_col=80
    )
    assert match_row == 3

    assert (
        mock_engine.find_text_in_block(
            pane_content, "Missing", row=1, col=1, end_row=4, end_col=80
        )
        is None
    )
