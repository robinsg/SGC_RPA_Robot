...
def test_run_tmux_success(engine):
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_any_call(
            ["tmux", "display-message", "hello"], capture_output=True, text=True
        )
...
def test_move_cursor(engine):
    with patch.object(engine, "get_cursor_position", return_value=(1, 1)), patch.object(
        engine, "run_tmux"
    ) as mock_run:
...
def test_wait_for_text_retry(engine):
    with patch.object(engine, "capture_pane") as mock_capture, patch(
        "time.sleep"
        ...
        mock_time.side_effect = [
            ...
            100.4,
            100.5,
        ]  # start, check1, sleep, check2, sleep, check3
...
def test_wait_for_text_timeout(engine):
    with patch.object(engine, "capture_pane", return_value="never found"), patch(
        "time.sleep"
...
def test_search_extract_and_send_action(engine):
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "wait_for_text_internal", return_value=(True, pane_content)
    ), patch.object(engine, "run_tmux") as mock_run, patch.object(
        engine, "capture_pane", return_value=pane_content
    ):
...
def test_extract_at_cursor_and_send_action(engine):
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "get_cursor_position", return_value=(1, 7)
    ), patch.object(engine, "capture_pane", return_value=pane_content), patch.object(
        engine, "run_tmux"
    ) as mock_run:
...
def test_engine_run_various_actions(engine, tmp_path):
    ...
    actions = [
        ...
        PressKeyIfTextPresentAction(
            type="press_key_if_text_present", text="ready", key="F3"
        ),
    ...
    with patch.object(engine, "check_session_exists", return_value=True), patch.object(
        engine, "run_tmux", side_effect=mock_run_tmux_impl
    ) as mock_run_tmux, patch.object(
        engine, "capture_pane", return_value=pane_content
    ), patch(
        "time.sleep"
    ), patch(
        ...
        # Verify calls
        mock_run_tmux.assert_any_call(
            ["send-keys", "-l", "-t", engine.session, "input"]
        )
...
def test_terminate_session(engine):
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_any_call(
            ["tmux", "kill-session", "-t", engine.session], capture_output=True
        )
        ...
        engine.terminate_session()
        assert not any(
            call[0][0] == ["tmux", "kill-session", "-t", engine.session]
...
def test_engine_run_session_not_found(engine):
    with patch.object(engine, "check_session_exists", return_value=False), patch(
        "robot_py.engine.logger.error"
    ) as mock_log_error:
        engine.run()
        mock_log_error.assert_any_call(
            f"Error: Tmux session '{engine.session}' not found."
        )
...
def test_capture_debug_screen(engine, tmp_path, monkeypatch):
    ...
    with patch.object(engine, "capture_pane", return_value=pane_content), patch(
        "os.makedirs"
